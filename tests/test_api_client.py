...
class ApiClientTests(unittest.TestCase):
        ...
        for raw_data in (b"NaN", b"Infinity", b"-Infinity", b"1e999", b'{"x":1,"x":2}'):
            with self.subTest(raw_data=raw_data):
                ...
                with self.assertRaisesRegex(ApiProtocolError, "invalid JSON"):
                    ApiClient(ClientConfig("http://localhost/api/v1"), opener).request("GET", "health")
    ...
    def test_rejects_url_whitespace_before_urlsplit_can_strip_it(self) -> None:
        for url in (
            "\nhttp://localhost/api/v1", "http://local\thost/api/v1",
            "http://localhost/api/v1\n", "http://localhost/api v1",
        ):
            with self.subTest(url=url), self.assertRaises(ValueError):
                ClientConfig(url)

    def test_encodes_query_and_validates_v1_envelope(self) -> None:
        opener = Mock()
        opener.open.return_value = FakeResponse(envelope({"ok": True}))
        client = ApiClient(ClientConfig("http://localhost:8001/api/v1/", 4), opener)
        ...
        self.assertEqual(
            request.full_url,
            "http://localhost:8001/api/v1/market/quotes?symbols=BRK.B%2CAAPL",
        )
        ...
        for timeout in (0, float("inf"), float("nan")):
            with self.subTest(timeout=timeout):
                with self.assertRaisesRegex(ValueError, "timeout"):
                    ClientConfig("http://localhost/api/v1", timeout)
        with self.assertRaisesRegex(ValueError, "base URL"):
            ClientConfig("http://user:secret@localhost/api/v1", 1)
        with self.assertRaisesRegex(ValueError, "invalid port"):
            ClientConfig("http://localhost:notaport/api/v1", 1)
        with self.assertRaisesRegex(ValueError, "base URL"):
            ClientConfig("http://local host/api/v1", 1)

        opener = Mock()
        opener.open.return_value = FakeResponse({"unrelated": True})
        with self.assertRaisesRegex(ApiProtocolError, "v1 envelope"):
            ApiClient(ClientConfig("http://localhost/api/v1"), opener).request(
                "GET", "health"
            )
    ...
    def test_maps_timeout_and_http_errors(self) -> None:
        ...
        opener.open.side_effect = HTTPError(
            "http://localhost/api/v1/health", 422, "error", {}, body
        )
        with self.assertRaises(ApiHttpError) as error:
            ApiClient(ClientConfig("http://localhost/api/v1"), opener).request(
                "GET", "health"
    ...
    def test_maps_interrupted_http_error_body_to_transport_error(self) -> None:
        ...
        error = HTTPError(
            "http://localhost/api/v1/health", 500, "error", {}, body
        )
