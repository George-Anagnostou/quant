...
class RecordRepository:
    ...
    def put(self, kind: str, key: str, payload: dict, *, after_insert=None):
        ...
        with database_connection(self.path) as connection:
            ...
            return self.put_in_connection(connection, kind, key, payload, after_insert=after_insert)

    def put_in_connection(self, connection, kind, key, payload, *, after_insert=None):
        """Append an immutable record within an already owned write transaction."""
        ...
        return self._decode(connection.execute("SELECT * FROM records WHERE id=?", (identifier,)).fetchone())
